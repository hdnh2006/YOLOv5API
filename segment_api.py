    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        if save_txt:
            if os.path.exists(txt_path + '.txt'):
                result = pd.read_csv(txt_path + '.txt', sep =" ",names = ["class","x","y","w","h","conf"], header = None, usecols=range(6)) # it will return just bboxes
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5s-seg.pt', help='model path or triton URL')
